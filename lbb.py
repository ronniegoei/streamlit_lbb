...
df.drop(columns=['No', 'Nama_Toko'], inplace=True)

# Drop rows with specific substrings in 'Nama_Barang' and reset index
pattern = 'Pete Kupas|Pete Papan'
df = df[~df['Nama_Barang'].str.contains(pattern, case=False, na=False)].reset_index(drop=True)

# Set the index to start from 1
df.index = df.index + 1

# Convert columns to numeric, coerce errors to NaN
numeric_columns = ['Ratio_Harga_per_100_Gram', 'Berat_dalam_gram', 'Harga']
...
with col2:
    ...
    fig = px.scatter(df.head(10), x='Nama_Barang', y='Ratio_Harga_per_100_Gram', text='Nama_Barang', size='Ratio_Harga_per_100_Gram', color='Nama_Barang')
    # Customize the chart (optional)
    fig.update_traces(textposition='top center')
    fig.update_layout(xaxis_title='Nama_Barang', yaxis_title='Harga per 100 Gram')
...
if 'checked' not in st.session_state:
    st.session_state.checked = {product: True for product in df['Nama_Barang'].unique()}
...
sorted_products = sorted(df['Nama_Barang'].unique())
...
df_filtered = df[~df['Nama_Barang'].isin(excluded_products)]
...
problem += pulp.lpSum([
    quantities[i] * (weight_importance * df_filtered.loc[i, 'Berat_dalam_gram'] + price_importance * df_filtered.loc[i, 'Harga'])
    for i in df_filtered.index
])

# Define the constraint: total weight must be less than or equal to max_weight
problem += pulp.lpSum([quantities[i] * df_filtered.loc[i, 'Berat_dalam_gram'] for i in df_filtered.index]) <= max_weight
...
if pulp.LpStatus[problem.status] == 'Optimal':
    ...
    for i in df_filtered.index:
        if quantities[i].varValue > 0:
            results.append({
                'Nama_Barang': df_filtered.loc[i, 'Nama_Barang'],
                'Optimal Units': quantities[i].varValue,
                'Total Weight': quantities[i].varValue * df_filtered.loc[i, 'Berat_dalam_gram'],
